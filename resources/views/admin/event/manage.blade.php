	...
	@endif
		<div class="card">
			...
			<div class="header bg-project">
				<h2 class="">Manage Service <a href="{{url('admin/event/sort')}}"><i class="material-icons pull-right">view_module</i></a></h2>
			</div>
			...
			<div class="body table-wrapper">
				<table class="table table-bordered table-responsive table-striped table-hover js-basic-example dataTable">
					...
					<tbody>
						...
						<tr>
							<td>
								<img height="50px" src="{{$event->imageUrl()}}">
							</td>
							<td>{{str_limit($event->title,60)}}<br/>{{str_limit($event->title_ar,60)}}</td>
							<td>{{str_limit($event->content,60)}}<br/>{{str_limit($event->content_ar,60)}}</td>
