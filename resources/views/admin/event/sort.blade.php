...
<div class="row">
	...
	<div class="card">
		<div class="header bg-project">
			<h2 class="">Manage Event<a href="{{url('admin/event')}}"><i class="material-icons pull-right">view_list</i></a></h2>
		</div>
	</div>
	<div class="connectedSortable">
		@foreach($events as $event)

		<div id="sort_{{$event->id}}" class="col-md-4 col-sm-6 col-xs-12">
			<div class="card">
				<div class="header" >
					<h2 style="white-space: nowrap;overflow:hidden">
						<span>{{str_limit($event->title,40)}}</span>
						<small>{{str_limit($event->content,60)}}</small>
					</h2>
					...
					<ul class="header-dropdown m-r--5">
						<li class="dropdown">
							...
							<ul class="dropdown-menu pull-right">
								<li><a href="{{url('admin/event/edit/'. $event->id)}}">Edit Event</a></li>
								<li><a href="{{url('admin/event/'.$event->id)}}" onclick="event.preventDefault();
						...
						<div  id="carousel-{{$event->id}}"  class="carousel slide" data-ride="carousel">
							<div class="carousel-inner" role="listbox">
								<div style="min-height:100px;" class="item active">
									<img style="margin:auto;" src="{{url(App\Models\Event::IMAGE_LOCATION.'/'.$event->image)}}" />
								</div>
							</div>
						</div>
					</div>
		...
		<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
			<div class="card">
				<div class="body">
					No data to display.
					<a href="{{url('admin/event/add')}}" class="btn btn-info pull-right">Add Event</a>
				</div>
			</div>
		</div>
		@endif
	...
	<script type="text/javascript">
    	...
    	$(".connectedSortable").on( "sortupdate", function( event, ui ) {
    		...
    		$.post("{{url('admin/event/sort')}}", order)
    	});
    </script> 
    {{--Sorting Ends--}}
