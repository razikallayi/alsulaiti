...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="card">
			...
			<h2 class="">@if(isset($client)) Edit @else Add @endif Client</h2>
			</div>
			<div class="body">

			@if(isset($client))
			<form id="form_validation" method="POST" action="{{url('admin/client/update/'.$client->id)}}" enctype="multipart/form-data">
				{{csrf_field()}}
				{{ method_field('PUT') }}
			@else
			<form id="form_validation" method="POST" action="{{url('admin/client')}}" enctype="multipart/form-data">
			@endif
			{{csrf_field()}}
					...
					<div class="col-sm-12">
						...
						<div class="form-group">
							<div class="form-line">
								<input type="url" value="{{$client->link or old('link')}}" name="link" maxlength="255" class="form-control">
							</div>
						</div>
					</div>
		...
		<div class="col-sm-12">
          ...
          <div id="result" class="connectedSortable">
            @if(isset($client) && null != $client->imageUrl())
            <input type="hidden" id="image-input-{{substr($client->image,0,-4)}}" name = "image[]" value="{{$client->image}}">
            <div id="image-preview-{{substr($client->image,0,-4)}}" class="col-lg-3 col-md-4 col-sm-6 m-t-30 sort_handle" style="min-height:100px">
              <span>
            {{--     <button type="button" style="float:right;" onclick="whyte.deleteImage('{{$client->image}}')" class="btn btn-xs  waves-effect btn-danger pull-right"><i class="material-icons">close</i></button> --}}
                
                <img class="img-responsive sortable_image" style="width:100%" src="{{$client->imageUrl()}}">
              </span>
            </div>
           @elseif(null != old('image')[0])
              <input type="hidden" id="image-input-{{substr(old('image')[0],0,-4)}}" name = "image[]" value="{{old('image')[0]}}">
              <div id="image-preview-{{substr(old('image')[0],0,-4)}}" class="col-md-3 m-t-30" style="min-height:100px"><span><img class="img-responsive" src="{{url(App\Models\Client::IMAGE_LOCATION."/".old('image')[0])}}"></span></div>
            @endif
          </div>
				...
				<div class="col-md-4 col-sm-12">
					...
					<div class="form-group">
						@php
						$checked=" checked ";
						if(isset($client) && $client->is_published){
							$checked = " checked ";
						}
						else{
...
@parent
  ...
  <script>
    $(function() {
      whyte={};
      whyte.imageWidth = 370;
      whyte.imageHeight = 170;
      whyte.storageLocation= "{{App\Models\Client::IMAGE_LOCATION}}";
      whyte.postUrl = "{{url('admin/client/image')}}";
