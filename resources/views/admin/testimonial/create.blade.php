...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="card">
			...
			<h2 class="">@if(isset($testimonial)) Edit @else Add @endif Testimonial</h2>
			</div>
			<div class="body">

			@if(isset($testimonial))
			<form id="form_validation" method="POST" action="{{url('admin/testimonial/update/'.$testimonial->id)}}" enctype="multipart/form-data">
				{{csrf_field()}}
				{{ method_field('PUT') }}
			@else
			<form id="form_validation" method="POST" action="{{url('admin/testimonial')}}" enctype="multipart/form-data">
			@endif
			{{csrf_field()}}
          ...
          <label>Client Name</label>
            <div class="form-group ">
              <div class="form-line">
                <input type="text" value="{{$testimonial->client_name or old('client_name')}}" name="client_name" maxlength="191" " class="form-control" required="">
              </div>
            </div>
          </div>
          <div class="col-sm-6">
          <label>Client Name Arabic</label>
            <div class="form-group ">
              <div class="form-line">
                <input type="text" value="{{$testimonial->client_name_ar or old('client_name_ar')}}" name="client_name_ar" maxlength="191" " class="arabic_input form-control">
              </div>
            </div>
          </div>
          ...
          <label>Content</label>
            <div class="form-group">
              <div class="form-line">
                <textarea name="content" rows="2" class="form-control htmlEditor">{{$testimonial->content or old('content')}}</textarea>
              </div>
            </div>
          </div>
          <div class="col-sm-6">
          <label>Content Arabic</label>
            <div class="form-group">
              <div class="form-line">
                <textarea name="content_ar" rows="2" class="form-control arabic_input htmlEditor">{{$testimonial->content_ar or old('content_ar')}}</textarea>
              </div>
            </div>
          </div>
          ...
          <div id="result" class="connectedSortable">
            @if(isset($testimonial) && null != $testimonial->imageUrl())
            <input type="hidden" id="image-input-{{substr($testimonial->image,0,-4)}}" name = "image[]" value="{{$testimonial->image}}">
            <div id="image-preview-{{substr($testimonial->image,0,-4)}}" class="col-lg-3 col-md-4 col-sm-6 m-t-30 sort_handle" style="min-height:100px">
              <span>
            {{--     <button type="button" style="float:right;" onclick="whyte.deleteImage('{{$testimonial->image}}')" class="btn btn-xs  waves-effect btn-danger pull-right"><i class="material-icons">close</i></button> --}}
                
                <img class="img-responsive sortable_image" style="width:100%" src="{{$testimonial->imageUrl()}}">
              </span>
            </div>
            @elseif(null != old('image')[0])
              <input type="hidden" id="image-input-{{substr(old('image')[0],0,-4)}}" name = "image[]" value="{{old('image')[0]}}">
              <div id="image-preview-{{substr(old('image')[0],0,-4)}}" class="col-md-3 m-t-30" style="min-height:100px"><span><img class="img-responsive" src="{{url(App\Models\Testimonial::IMAGE_LOCATION."/".old('image')[0])}}"></span></div>
            @endif
          </div>
...
<div class="col-md-4 col-sm-12">
          ...
          <div class="form-group">
            @php
            $checked=" checked ";
            if(isset($testimonial) && $testimonial->is_published){
              $checked = " checked ";
            }
            else{
...
@parent
  ...
  <script>
    $(function() {
      ...
      whyte.imageHeight = 200;
      whyte.storageLocation= "{{App\Models\Testimonial::IMAGE_LOCATION}}";
      whyte.postUrl = "{{url('admin/testimonial/image')}}";
