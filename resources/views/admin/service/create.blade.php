...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="card">
			...
			@if(isset($service))
			<form id="form_validation" method="POST" action="{{url('admin/service/update/'.$service->id)}}" enctype="multipart/form-data">
				{{csrf_field()}}
				{{ method_field('PUT') }}
			@else
			<form id="form_validation" method="POST" action="{{url('admin/service')}}" enctype="multipart/form-data">
			@endif
			{{csrf_field()}}
          ...
          <label>Title</label>
            <div class="form-group ">
              <div class="form-line">
                <input type="text" value="{{$service->title or old('title')}}" name="title" maxlength="191" " class="form-control" required="">
              </div>
            </div>
          </div>
          <div class="col-sm-6">
          <label>Title Arabic</label>
            <div class="form-group ">
              <div class="form-line">
                <input type="text" value="{{$service->title_ar or old('title_ar')}}" name="title_ar" maxlength="191" " class="arabic_input form-control">
              </div>
            </div>
          </div>
          ...
          <label>Content</label>
            <div class="form-group">
              <div class="form-line">
                <textarea name="content" rows="5" class="form-control htmlEditor">{{$service->content or old('content')}}</textarea>
              </div>
            </div>
          </div>
          <div class="col-sm-6">
          <label>Content Arabic</label>
            <div class="form-group">
              <div class="form-line">
                <textarea name="content_ar" rows="5" class="form-control arabic_input htmlEditor">{{$service->content_ar or old('content_ar')}}</textarea>
              </div>
            </div>
          </div>
          ...
          <div id="result" class="connectedSortable">
            @if(isset($service) && null != $service->imageUrl())
            <input type="hidden" id="image-input-{{substr($service->image,0,-4)}}" name = "image[]" value="{{$service->image}}">
            <div id="image-preview-{{substr($service->image,0,-4)}}" class="col-lg-3 col-md-4 col-sm-6 m-t-30 sort_handle" style="min-height:100px">
              <span>
            {{--     <button type="button" style="float:right;" onclick="whyte.deleteImage('{{$service->image}}')" class="btn btn-xs  waves-effect btn-danger pull-right"><i class="material-icons">close</i></button> --}}
                
                <img class="img-responsive sortable_image" style="width:100%" src="{{$service->imageUrl()}}">
              </span>
            </div>
            @elseif(null != old('image')[0])
              <input type="hidden" id="image-input-{{substr(old('image')[0],0,-4)}}" name = "image[]" value="{{old('image')[0]}}">
              <div id="image-preview-{{substr(old('image')[0],0,-4)}}" class="col-md-3 m-t-30" style="min-height:100px"><span><img class="img-responsive" src="{{url(App\Models\Service::IMAGE_LOCATION."/".old('image')[0])}}"></span></div>
            @endif
          </div>
...
<div class="col-md-4 col-sm-12">
          ...
          <div class="form-group">
            @php
            $checked=" checked ";
            if(isset($service) && $service->is_published){
              $checked = " checked ";
            }
            else{
...
@parent
  ...
  <script>
    $(function() {
      whyte={};
      whyte.imageWidth = 570;
      whyte.imageHeight = 461;
      whyte.storageLocation= "{{App\Models\Service::IMAGE_LOCATION}}";
      whyte.postUrl = "{{url('admin/service/image')}}";
